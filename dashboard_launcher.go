...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  header h1    { font-size: 2rem; color: #00d4aa; letter-spacing: -.5px; }
  ...
  .check { display: flex; align-items: center; gap: .5rem; color: #e6edf3; font-size: .85rem; }
  ...
  .primary   { background: #00d4aa; border-color: #00d4aa; color: #0d1117; }
  .primary:hover { background: #00b894; }
  ...
  .command { background: #0d1117; border: 1px solid #21262d; border-radius: 6px; padding: .85rem; color: #e6edf3; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size: .78rem; line-height: 1.55; word-break: break-all; }
  ...
  .mini .ck { margin-bottom: .2rem; }
  .mini .cv { font-size: .82rem; }

  /* ── Test history ── */
  .tests { display: flex; flex-direction: column; gap: .75rem; }
  .test-card { padding: 1rem; background: #0d1117; border: 1px solid #21262d; border-radius: 8px; }
  .test-card-head { display: flex; justify-content: space-between; align-items: center; margin-bottom: .6rem; }
  .test-pid { font-size: .75rem; color: #8b949e; font-family: monospace; }
  .test-time { font-size: .78rem; color: #8b949e; margin-bottom: .5rem; }
  .test-cmd { font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size: .75rem; color: #8b949e; line-height: 1.45; word-break: break-all; margin-bottom: .75rem; }
  .badge { display: inline-flex; align-items: center; gap: .4rem; font-size: .7rem; padding: .2rem .6rem; border-radius: 99px; font-weight: 700; text-transform: uppercase; letter-spacing: .5px; }
  ...
  .badge-finished { background: #3a301a; color: #d29922; }
  ...
  .browser-warn strong { color: #e6b84a; }

  /* ── Misc ── */
  .err { color: #f85149; font-size: .82rem; }
  .ok  { color: #3fb950; font-size: .82rem; }
...
<div class="container">
  ...
  <header>
    <div>
      ...
      <p class="meta">HTTP load test launcher &nbsp;·&nbsp; port %d</p>
    </div>
  ...
  <div class="cards">
    <div class="card"><div class="card-label">Target RPS</div><div class="card-value" id="sumRps">100</div></div>
    <div class="card"><div class="card-label">Duration</div><div class="card-value yellow" id="sumDuration">30s</div></div>
    <div class="card"><div class="card-label">Endpoints</div><div class="card-value" id="sumEndpoints">1</div></div>
    <div class="card"><div class="card-label">Mode</div><div class="card-value green" id="sumMode">HTTP</div></div>
  </div>
  ...
  <div class="layout">
    <main>
      <section class="section">
        ...
        <div class="check-row">
          <label class="check"><input id="browserMode" type="checkbox"> Modo browser (headless Chrome)</label>
          <span class="section-note">requer Chrome instalado</span>
        </div>
        <div class="browser-warn" id="browserWarn">
          <strong>Atenção:</strong> este modo abre abas reais no Google Chrome/Chromium via CDP.
          Requer Chrome instalado na máquina. Para teste de carga HTTP simples, deixe desmarcado —
          o modo padrão é mais rápido e suporta muito mais RPS sem precisar de browser.
        </div>
...
<template id="endpointTpl">
  <div class="endpoint">
    <div class="endpoint-head">
      ...
      <button class="danger remove" type="button">Remover</button>
    </div>
...
function renumber() {
  [...endpointsEl.children].forEach((n, i) => {
    ...
    n.querySelector('.endpoint-title').innerHTML =
      '<span class="method-chip">' + method + '</span><span>Endpoint ' + (i + 1) + '</span>';
  });
  [...endpointsEl.querySelectorAll('.remove')].forEach(
    b => b.style.display = endpointsEl.children.length > 1 ? 'inline-block' : 'none'
  );
  updatePreview();
}
...
function updatePreview() {
  ...
  if (browser) {
    parts.push('-browser', '-users', $('users').value || '10');
    if ($('thinkTime').value) parts.push('-think-time', $('thinkTime').value);
    if ($('thinkMax').value)  parts.push('-think-max',  $('thinkMax').value);
  }
  if (eps.length === 1) {
    parts.push('-method', eps[0].method, '-url', eps[0].url || '<url>');
    if (eps[0].body) parts.push('-body', eps[0].body);
  } else {
    parts.push('-scenario', 'scenario.json');
  }
...
$('browserMode').onchange   = e => {
  const on = e.target.checked;
  document.getElementById('browserFields').style.display = on ? 'grid' : 'none';
  document.getElementById('browserWarn').style.display   = on ? 'block' : 'none';
  updatePreview();
};
['rps','duration','rampUp','timeout','maxErrorRate','token','basic','headers','users','thinkTime','thinkMax']
...
async function refreshTests() {
  ...
  el.innerHTML = tests.length ? '' : '<div style="color:#8b949e;font-size:.82rem">Nenhum teste ainda.</div>';
  tests.forEach(t => {
    const card     = document.createElement('div');
    card.className = 'test-card';
    const finished    = t.status === 'finished';
    const badgeClass  = finished ? 'badge badge-finished' : 'badge badge-running';
    const badgeText   = finished ? 'finalizado' : 'em andamento';
    card.innerHTML =
      '<div class="test-card-head">' +
        '<span class="' + badgeClass + '">' + badgeText + '</span>' +
        '<span class="test-pid">PID ' + t.pid + '</span>' +
      '</div>' +
      '<div class="test-time">' + new Date(t.startedAt).toLocaleString() + '</div>' +
      '<div class="test-cmd">' + t.command + '</div>' +
      '<a class="btn secondary" target="_blank" href="' + t.liveURL + '">Abrir ao vivo</a>';
    el.appendChild(card);
  });
}
...
async function startTest() {
  ...
  msg.className = 'ok'; msg.textContent = 'Teste iniciado.';
  refreshTests();
}
